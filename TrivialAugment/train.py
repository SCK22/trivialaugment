...
def run_epoch(
    ...
    if C.get().get('load_sample_pairing_batch', False) and sample_pairing_loader is not None:
        ...
        aug_lib.blend_images = [
            transforms.ToPILImage()(sample_pairing_loader.denorm(ti))
            for ti in next(sample_pairing_iter)[0]
        ]
    for batch in logging_loader:  # logging loader might be a loader or a loader wrapped into tqdm
        ...
        if C.get().get('load_sample_pairing_batch', False) and sample_pairing_loader is not None:
            try:
                ...
                print(
                    """Blend images iterator ended. If this is printed twice per loop,
        ...
        just_communicated_grad = (
            steps % communicate_grad_every == 1
        )  # also is true in first step of each epoch
        ...
        if optimizer:
            ...
            if (steps - 1) % C.get().get('step_optimizer_every', 1) == C.get().get(
                'step_optimizer_nth_step', 0
            ):  # default is to step on the first step of each pack
                optimizer.step()
        ...
        if steps % 2 == 0:
            metrics.add(
                'eval_top1', top1.item() * len(data)
            )  # times 2 since it is only recorded every sec step
            eval_cnt += len(data)
        cnt += len(data)
        if verbose:
...
def train_and_eval(
    ...
    writers = [
        ...
        for x in ['train', 'valid', 'test', 'testtrain']
    ]

    aug_lib.set_augmentation_space(
        C.get().get('augmentation_search_space', 'standard'),
    ...
    (
        ...
        started_with_spawn=C.get()['started_with_spawn'],
        summary_writer=writers[0],
    )

    # create a model & an optimizer
    model = get_model(
        C.get()['model'], C.get()['batch'], num_class(C.get()['dataset']), writer=writers[0]
    )
    if worldsize > 1:
    ...
    if C.get()['optimizer']['type'] == 'sgd':
        ...
        optimizer = optim.Adam(
            model.parameters(),
            lr=C.get()['lr'],
            betas=(C.get()['optimizer'].get('momentum', 0.9), 0.999),
        )
    else:
        ...
        if epoch % 20 == 0 or epoch == max_epoch:
            ...
            if metric == 'last' or rs[metric]['top1'] > best_top1:
                ...
                for key, setname in itertools.product(
                    ['loss', 'top1', 'top5'], ['train', 'test', 'testtrain']
                ):
                ...
                reporter(
                    loss_valid=rs['test']['loss'],
                    top1_valid=rs['test']['top1'],
                ...
                if (
                    save_path
                    and C.get().get('save_model', True)
                    and (worldsize <= 1 or torch.distributed.get_rank() == 0)
                ):
                    ...
                    torch.save(
                        ...
                        save_path.replace(
                            '.pth',
                            '_e%d_top1_%.3f_%.3f' % (epoch, rs['train']['top1'], rs['test']['top1'])
                            + '.pth',
                        ),
                    )
...
def spawn_process(
    ...
    args,
    config_path=None,
    communicate_results_with_queue=None,
    local_rank=None,
):
    ...
    if worldsize:
        assert (
            worldsize == C.get()['gpus']
        ), f"Did not specify the number of GPUs in Config with which it was started: {worldsize} vs {C.get()['gpus']}"
    else:
    ...
    with open(f"outputs/{args.tag}.json", "w") as f:
        json.dump(
            result,
        )
    elapsed = time.time() - t
...
def run_from_py(dataroot, config_dict, save=''):
    ...
    with tempfile.NamedTemporaryFile(mode='w+') as f, tempfile.NamedTemporaryFile() as result_file:
        ...
        result_queue = mp.get_context('spawn').Value('d', 0.0)
        if world_size > 1:
            outcome = mp.spawn(
                spawn_process,
...
if __name__ == '__main__':
    ...
    if args.local_rank is None:
        ...
        # os.makedirs("tmp/samshpopt/", exist_ok =True)
